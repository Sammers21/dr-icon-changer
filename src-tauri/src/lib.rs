...
async fn apply_dr_icon(
    ...
    fs::write(&target_file, image_data)
...
async fn reset_dr_icon(
    ...
    if target_file.exists() {
        ...
        Ok(format!("Icon reset successfully, removed {:?}", target_file))
    } else {
...
async fn reset_all_icons(
    ...
    if icons_folder.exists() {
        ...
        Ok(format!("All icons reset successfully, removed {:?}", icons_folder))
    } else {
...
pub fn run() {
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![get_wow_versions, apply_dr_icon, reset_dr_icon, reset_all_icons])
